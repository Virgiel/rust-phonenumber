...
fn metadata<R: BufRead>(reader: &mut Reader<R>) -> Result<Vec<Metadata>, error::LoadMetadata> {
    ...
    loop {
        match reader.read_event_into(&mut buffer)? {
            ...
            Event::End(ref e) if e.name().0 != b"phoneNumberMetadata" => {
                return Err(
                    error::Metadata::MismatchedTag(str::from_utf8(e.name().0)?.into()).into(),
                )
            }

            Event::End(ref e) if e.name().0 == b"phoneNumberMetadata" => return Ok(result),
...
fn territories<R: BufRead>(reader: &mut Reader<R>) -> Result<Vec<Metadata>, error::LoadMetadata> {
    ...
    loop {
        match reader.read_event_into(&mut buffer)? {
            ...
            Event::End(ref e) if e.name().0 == b"territories" => return Ok(result),
            Event::End(ref e) => {
                return Err(
                    error::Metadata::MismatchedTag(str::from_utf8(e.name().0)?.into()).into(),
                )
...
fn territory<R: BufRead>(
    ...
    for attr in e.attributes() {
        ...
        match (str::from_utf8(key.0)?, str::from_utf8(&value)?) {
    ...
    loop {
        match reader.read_event_into(&mut buffer)? {
            ...
            Event::End(ref e) if e.name().0 == b"territory" => return Ok(meta),
...
fn descriptor<R: BufRead>(
    ...
    loop {
        match reader.read_event_into(&mut buffer)? {
            ...
            Event::End(ref e) if e.name().0 == name => return Ok(descriptor),
...
fn format<R: BufRead>(
    ...
    loop {
        match reader.read_event_into(&mut buffer)? {
            ...
            Event::End(ref e) if e.name().0 == name => {
...
fn text<R: BufRead>(reader: &mut Reader<R>, name: &[u8]) -> Result<String, error::LoadMetadata> {
    ...
    loop {
        match reader.read_event_into(&mut buffer)? {
            ...
            Event::End(ref e) if e.name().0 == name => return Ok(result),
